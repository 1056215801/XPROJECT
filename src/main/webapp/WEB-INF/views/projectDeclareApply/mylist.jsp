...
<script type="text/javascript">
  function applyView(applyId){
	  window.location.href="${ctx}/projectDeclareApply/view?applyId=" + applyId;
  }
  function infoView(declareId){
	  window.location.href="${ctx}/projectDeclareInfo/view?declareId=" + declareId;
  }
						...
						<table id="exampleTableToolbar" data-mobile-responsive="true">
							...
							<tbody>
								<c:forEach items="${page}" var="info">
									...
									<td>${info.declareTitle}</td>
									<td>张三</td>
									<td>
									<c:if test="${info.declareStatus==1}">
									申报中
									</c:if>
									<c:if test="${info.declareStatus==0}">
									已结束
									</c:if>
								    ...
								    <td>
										<img alt="" src="${ctx}/static/img/detail.png" title="详情" style="width: 22px;height: 22px;cursor: pointer;" onclick="applyView('${info.applyId}')"/>
										<img alt="" src="${ctx}/static/img/original.png" title="查看项目申报原文" style="width: 22px;height: 22px;cursor: pointer;" onclick="infoView('${info.declareId}')"/>
									</td>
								</tr>
								</c:forEach>
