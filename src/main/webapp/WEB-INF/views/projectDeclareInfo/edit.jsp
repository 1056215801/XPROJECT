...
<body class="gray-bg">
					...
					<div class="ibox-content">
						<form method="post" class="form-horizontal"
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" name="declareTitle" id="declareTitle"
										value="${projectDeclareInfo.declareTitle}" />
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<select class="form-control m-b" name="declareGrade">
										...
										<option value='区级' ${projectDeclareInfo.declareGrade == '区级'?"selected":"" }>区级</option>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
								 <script name="declareText" id="editors" type="text/plain" style="width:100%;height: 300px;">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<div style="float: left;">
										<img alt="" src="${projectDeclareInfo.declareImage}" id="showPicUrl" style="width: 100px; height: 100px;" />
									</div>
									<div style="float: left; margin-left: 15px;">
										<p>图片小于2M ( jpg、gif、png)，尺寸为1080*640px</p>
										<input type="file" name="picFile" data-no-uniform="true"  id="pic_uploads" multiple="true" />
									</div>
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<textarea  name='declareRemark' id='declareRemark' cols='60' rows='4' style='width: 70%; height: 150px;' placeholder="若不设置时间节点，请填写备注，例'常年受理'">${projectDeclareInfo.declareRemark }</textarea>
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								<label class="col-sm-2 control-label">公示是否公开</label>
								<div class="col-sm-10">
									<div class="radio i-checks">
									<c:if test="${projectDeclareInfo.publicityOpen!=null}">
                                      <label>
                                        <input type="radio" name="publicityOpen" value="1" ${projectDeclareInfo.publicityOpen == '1'?"checked":"" }>是
                                      </label>
                                      <label>
                                        <input type="radio" name="publicityOpen" value="0"  ${projectDeclareInfo.publicityOpen == '0'?"checked":"" } >否
                                      </label>
									</c:if>
									<c:if test="${projectDeclareInfo.publicityOpen==null}">
                                      <label>
                                        <input type="radio" name="publicityOpen" value="1" checked="checked">是
                                      </label>
                                      <label>
                                        <input type="radio" name="publicityOpen" value="0" >否
                                      </label>
									</c:if>
                               		</div>
								</div>
							...
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								<label class="col-sm-2 control-label">是否允许查看申报进度</label>
								<div class="col-sm-10">
								<div class="radio i-checks">
								<c:if test="${projectDeclareInfo.scheduleOpen!=null}">
                                      <label>
                                        <input type="radio" name="scheduleOpen" value="1" ${projectDeclareInfo.scheduleOpen == '1'?"checked":"" }>是
                                      </label>
                                      <label>
                                        <input type="radio" name="scheduleOpen" value="0"  ${projectDeclareInfo.scheduleOpen == '0'?"checked":"" } >否
                                      </label>
									</c:if>
									<c:if test="${projectDeclareInfo.scheduleOpen==null}">
                                      <label>
                                        <input type="radio" name="scheduleOpen" value="1" >是
                                      </label>
                                      <label>
                                        <input type="radio" name="scheduleOpen" value="0" checked="checked">否
                                      </label>
									</c:if>
								</div>
								</div>
							...
							<div class="form-group" >
								<div class="col-sm-4 col-sm-offset-2" >
									...
									<button class="btn btn-danger" id="previewBtn">预览</button>
								</div>
							</div>
						</form>
					</div>
    ...
    <script>
		...
		$(function() {
			...
			//附件编辑时展示
			if("${projectDeclareInfo.atIdList!=null}"){
				var atIdList = "${projectDeclareInfo.atIdList}";
			$.post("${ctx}/login/json/attachment/selectListByAttachmentIdList", "attachmentIdList="+atIdList, 
	    		function(datas) {
					...
					.uploadify(
							{
								...
								'onUploadSuccess' : function(file, data,
									...
									showPic(obj);
									$("#declareImage").val(obj.attachmentUrl);
								}
							});
			...
			$("#saveBtn").click(function() {
				...
				for(var i = 1;i<len;i++){
					...
					if($('#endTime_'+i).val()<$('#endTime_'+(i-1)).val()){
						alert('时间节点截止时间必须根据序号依次递增！');
						return false;
					}
				}
				}else{
					if($('#declareRemark').val()==""){
						alert('若没有时间节点，备注不能为空！');
						return false;
					}
				}
				$("#form").submit();
		...
		function addtr(){
			...
			$("#tab").append(
							"<tr id='"+squences+"' align='left'>"
									+ "<td><input type='hidden' name='nodeNum' id='"+nodeNumId+"'  value='"+ _len+ "' />"+ _len+ "</td>"
									+ "<td><input type='text' name='nodeTitle' id='"+nodeNameId+"' style='width: 80%'/></td>"
									+ "<td><input id='"+endTimeId+"' class='laydate-icon form-control layer-date' name='endTime' style='width: 60%' /></td>"
									+ "<td style='height: 60px;'><textarea name='remark' id='"+ reMarkId+ "' cols='60' rows='4' style='width: 70%; height: 100px;'></textarea></td>"
									+ "<td><a href='javascript:void(0);' title='删除' onclick='deltr("+squences+")'>[删除]</a></td>" + "</tr>"
									);
			endTime(squences);
			return squences;
		}
