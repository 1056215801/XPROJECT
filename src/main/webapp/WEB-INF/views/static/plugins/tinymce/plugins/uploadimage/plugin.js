...
tinymce.PluginManager.add('uploadimage', function (editor) {
    ...
    function selectLocalImages() {
        var dom = editor.dom;
        var input_f = $('<input type="file" name="attachment" accept="image/jpg,image/jpeg,image/png,image/gif" multiple="multiple">');
        input_f.on('change', function () {
            ...
            form.ajaxSubmit({
                ...
                success: function (data) {
                	var obj = eval('(' + data + ')');
                	if(obj.success) {
                         editor.selection.setContent(dom.createHTML('img', {src: obj.attachment.attachmentUrl}));
                    }
                }
            });
        });
