...
<script>
	...
	//获取调查列表
	function getList(pageNum, name, status) {// 134.225.53.139 117.21.209.17
		...
		$.ajax({
			url : "http://117.21.209.17:8080/OfficeAutomationPlatform/Spring/json/documents/documentsissuedinfo/viewListOfPhone",
			type : "get",
		data : {
			"pageNo" : pageNum,
			"pageSize" : '5',
		...
		success : function(result) {
			...
			if (result.errorCode == -1) {
				...
				for (var i = 0; i < list.length; i++) {
					var info = list[i];
					var _div = "";
					var logId = (info.logId == null)?'0':info.logId;
					_div += "<a href=\"${ctx}/documentIssued/newView?id="+info.id+"&logId=" + logId + "&logStatus=" + info.logStatus + "\" style=\"text-decoration:none;\">";
