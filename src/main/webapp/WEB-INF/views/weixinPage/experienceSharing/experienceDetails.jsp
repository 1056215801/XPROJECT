...
<script type="text/javascript">
	$(function(){
		...
		if(str.indexOf("main=1")!=-1){
			$("#back_a").attr("href","${ctx}/auth/page/mobile/weixin/experienceSharing/weixin/main");
		}
		 ...
		 $.ajax({
				...
				success:function(data){
					...
					for(var i = 0;i<value.length;i++){
						$('#attList').append(
								 "<li style='list-style-type:none;'><a style='color:#3FBA30;font-size:18px;' href='"+value[i].downloadUrl+"' target='_blank'>"+value[i].remoteName+"</a>["+ (value[i].attachmentSize/1024).toFixed(1) +"Kb]</li>"
								);
					}
				},
				error:function(){
	...
	function commentReply(replyId,replayUser){
		...
		if(${empty curAccountId}){//未登录
			layer.msg("亲，您还没有登录");
			setTimeout(function() {
				window.location.href=("${ctx}/experienceSharing/weixin/detail/${expShareInfo.expId}");
			}, 1000);
		}else{
	...
	function sendComment(){
		...
		if(${empty curAccountId}){//未登录
			layer.msg("亲，您还没有登录");
			setTimeout(function() {
				window.location.href=("${ctx}/experienceSharing/weixin/add/detail/${expShareInfo.expId}");
			}, 1000);
		}else{
			$("#commentMsg").val("");
			commentIndex=layer.open({
