		...
		<div class="main">
			...
			<div class="pro-right index3-right">
				...
				<div class="pro-tab" id="proTab1" style="display: none;">
					...
					<div class="price-detail" style="height: calc(100% - 226px);">
...
<script type="text/javascript">
				...
				if (result.success) {
					...
					for (var i = 0; i < result.typeList.length; i++) {
						...
						_div += '<div class="product-type">' + info.typeName
							+ '<i class="fa fa-angle-up fa-angle-down"></i></div>';
					...
					$(".product-type").on(
						...
						function(){
							...
							$(this).parent().addClass("on").siblings().removeClass("on").find("i").addClass(
								"fa-angle-down");
			...
			success : function(result){
				if (result.success) {
					...
					for (var i = 0; i < result.indicatorsList.length; i++) {
						...
						_div1 += '<span onclick="checkSelect1(this);"><i class="fa fa-square '+(i==0?'fa-check-square':'')+'" data-val="'+name+'" ></i>'
							+ name + '</span>'
					}
	...
	function loadData(type){
		var idsArr = [];
		var namesArr = [];
		$("#productItems span").each(function(){
			var ind = $(this).index();
			var txt = $(this).text();
			idsArr.push($(this).attr("data-val"));
