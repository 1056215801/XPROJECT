...
<%-- 	<div class="nav">
	...
	<div class="container-fluid content2">
		...
		<div class="pro-left">
				...
				<a class="searchBth" href="javascript:;" onclick="showSearchView()"><img width="18" style="margin-right:5px;" src="${ctx}/static/image/liData/search.png">搜索</a>
				<div class="seachView">
				     <div class="searchBar">
				        <div class="subBar">
					 ...
					 <div class="cancleSearch">
					 	<a href="javascript:;" onclick="canlceSearch()"><img width="15" style="margin-right:5px;" src="${ctx}/static/image/liData/cancle.png"><font>关闭</font></a>
					 </div>
				</div>
			</div>
			<div class="pro-right noBorder">
...
var type = "";
	...
	function changePage(e){
		$('#pageNum').val(e)
		if(type==0){
			getAssistExpertInfoList(e,6,0);
		}else if(type==1){
			getLiScienceType(e,1,0);
		}else{
			getIndustrialImage(e,6,0);
		}
	}
	function first(){
	...
	function pre(){
		...
		if(pageNum!=1){
			pageNum --;
			if(type==0){
				getAssistExpertInfoList(pageNum,6,0);
			}else if(type==1){
				getLiScienceType(pageNum,0);
			}else{
				getIndustrialImage(pageNum,6,0);
			}
		}
	}
	function next(){
	...
	function getIndustrialImage(pageNum,pageSize,type){
		...
		$.ajax({
			...
			success : function(result) {
				...
				$('#product2').text("产业峰会("+result.total+")");
	...
	function getLiScienceList(pageNum,pageSize,type) {
		...
		$.ajax({
			...
			success : function(result) {
				...
				$('#product1').text("科研机构("+result.total+")");
				$('#record').text("共"+result.total+"条记录");
				var allPage = Math.ceil(result.total/6);
				$('#font2').text('/'+allPage+'页');
				$('#lastPage').val(allPage);
				...
				if(result.total==0){
	 			...
	 			for(var i=0;i<infoList.length;i++){
					...
					_div += '<div class="middle-pic">'
					if(info.logoUrl){
						_div +=	 '<div class="expert-pic science-img" style="background-image:url('+info.logoUrl+')">'
					}else{
						_div +=	'<div class="expert-pic science-img" style="background-image:url(${ctx}/static/image/homePage_yc/kyjg1.png)">'
					}
					_div += '</div>'
					_div += '<div class="expert-infor">'
	...
	function getAssistExpertInfoList(pageNum,pageSize,type) {
		...
		$.ajax({
			...
			success : function(result) {
				var infoList = result.list;
				$('#product0').text("锂电专家("+result.total+")");
				$('#record').text("共"+result.total+"条记录");
				if(type==1){
					$('.hotSearch').hide();
				}
				...
				for(var i=0;i<infoList.length;i++){
					var info = infoList[i];
					var _div = "";
					_div += '<a class="dib expert-a" target="_blank" href="${ctx}/expert/detail/'+info.expertId+'?entranceId=3609009"><div class="middle-pic">'
					if(info.imageUrl){
