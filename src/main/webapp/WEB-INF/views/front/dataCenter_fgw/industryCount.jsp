...
<body>
	...
	<c:choose>
		<c:when test="${areaInfo.isProvince}">
			<input type="hidden" id="nextPageUrl" name="nextPageUrl" value="${ctx}/front/dataCenter/energyCount?orgId=${orgId}">
		</c:when>
		<c:otherwise>
		...
		<div class="data_contentRight" style="width:60%;">
			<div class="dataBar_3_left">
				<div class="dataContTitleBar">
					<span class="dataContTitle">按开发区排名</span>
...
<div class="btn_prev" onclick="window.location.href='${ctx}/front/dataCenter/economicCount?orgId=${orgId}'"></div>
<c:choose>
	<c:when test="${areaInfo.isProvince}">
		<div class="btn_next" onclick="window.location.href='${ctx}/front/dataCenter/energyCount?orgId=${orgId}'"></div>	
	</c:when>
	<c:otherwise>
