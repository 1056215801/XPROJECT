...
<style>
   ...
   .gray-bg {
    	background: url(${ctx}/static/image/cityDataCenter/jinx/bg.png);
	}
	...
	.tabCaption span:hover, .tabCaption span.on {
	    color: #FFF;
	}
	...
	<div class="drawContent" style="margin-top: 5px;">
					...
					<div class="tabContBar">
						...
						<div class="tabCont" id="tab-1-2">
							<div class="chartBar">
								...
								<div class="chartUnit">单位：万元/吨标准煤</div>
							</div>
						</div>
		...
		<div class="spanStyle" style="width: calc(45% - 10px);">
			<div class="blockStyle" style="height: 70%;">
				<div class="tabCaptionBar">
					<span class="tabCaption" id="companyList" style="cursor: pointer;"><!-- onclick="choiceCompany()" -->
						<div id="companyTitleName" style="display: inline-block; vertical-align: middle;"></div>
						<i class="fa fa-caret-down"></i>
					</span>
					<div class="company_list">
						<ul id="">
							<li data="201611220119148080182">江西洪达医疗器械集团有限公司</li>
							<li data="2016122911314080800504">江西鹏翔医疗设备集团有限公司</li>
						</ul>
					</div>
				</div>
				<div class="tabContBar">
			...
			<div class="blockStyle" style="height: calc(30% - 10px);">
				<div class="inlineBox">
					<%-- <div class="videoBar" id="video">
						<img id="video1" src="${ctx}/static/image/cityDataCenter/video1.gif" width="92%;" style="padding: 8px; display: none;">
						<img id="video2" src="${ctx}/static/image/cityDataCenter/video2.gif" width="92%;" style="padding: 8px; display: none;">
					</div> --%>
				...
				<div class="outsideBox">
					...
					<div class="tabContBar">
						...
						<div class="tabCont cpjg-table talign_c respondControl" id="tab-4-2">
							<div class="table-head">
								<span class="w70" style="text-align: left;">诉求名称</span>
...
<script type="text/javascript">
	$(function(){
		...
		$(".company_list li").on("click",function(){
			...
			window.location.href = "${ctx}/front/cityDataCenter/jxIndex/2?companyId=" + id;
		})
	...
	function goIndex(index){
		if(index==1){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/1';
		}else if(index==2){
			
		}else if(index==3){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/3';
		}else if (index==4){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/4';
		}else if(index==5){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/5';
		}else if (index==6){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/6';
		}else if(index==7){
			window.location.href= '${ctx}/front/cityDataCenter/jgsIndex/7';
		}
	...
	function choiceCompany(){
		layer.open({
			...
			shade: 0.3,
			area: ['900px', '675px'],
			content: '${ctx}/front/page/companyInfo/frame?parkId=1242&treeType=park' //iframe的url
		}); 
	};
			...
			success : function(result) {
				...
				if (result.success) {
					...
					if(result.data.taxRate != undefined && result.data.taxRate.xAxis.length != "" && result.data.taxRate.xAxis.length != undefined){
						...
						blueLineOption.series[0].name = "税负率";
					...
					if(result.data.productSalesRate != undefined && result.data.productSalesRate.xAxis.length != "" && result.data.productSalesRate.xAxis.length != undefined){
						...
						blueLineOption.series[0].name = "产品产销率";
			...
			success : function(result) {
				...
				if (result.success) {
					...
					if(result.companyInfo.companyName != "" &&  result.companyInfo.companyName != undefined){
						...
						if(result.companyInfo.companyName.indexOf("洪达") != -1){
							$("#video1").show();
							$("#video2").hide();
						}else if(result.companyInfo.companyName.indexOf("鹏翔") != -1){
							$("#video1").hide();
							$("#video2").show();
						}else{}
					}
