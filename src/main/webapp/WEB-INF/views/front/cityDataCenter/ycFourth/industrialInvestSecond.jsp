...
<body class="gray-bg">
	...
	<div class="content p15 fz0">
		...
		<div class="spanStyle" style="width: 80%;">
			<div class="spanStyle" style="width: 50%; height: calc(50% + 35px);">
			...
			<div class="spanStyle" style="width: calc(50% - 10px); height: calc(50% - 35px); margin-top: 70px;">
				<div class="outsideBox">
					...
					<div class="tabCaptionBar">
						<span class="tabCaption">项目进度一览</span>
					</div>
			...
			<div class="blockStyle" style="height: calc(50% - 10px - 35px); margin-top: 10px;">
				<div class="outsideBox">
					<img class="corner lt" src="${ctx}/static/image/cityDataCenter/jxNew/lt.png">
					<img class="corner rt" src="${ctx}/static/image/cityDataCenter/jxNew/rt.png">
					<img class="corner lb" src="${ctx}/static/image/cityDataCenter/jxNew/lb.png">
					...
					<div class="inlineBox">
						<div class="tabCaptionBar">
							<span class="tabCaption">各行业投资占比(亿元)</span>
						</div>
						<div class="tabContBar">
		...
		<div class="spanStyle" style="width: calc(20% - 10px);">
			<div class="blockStyle" style="height: calc(50% - 35px); margin-top: 70px;">
				<div class="outsideBox">
					<img class="corner lt" src="${ctx}/static/image/cityDataCenter/jxNew/lt.png">
					<img class="corner rt" src="${ctx}/static/image/cityDataCenter/jxNew/rt.png">
					<img class="corner lb" src="${ctx}/static/image/cityDataCenter/jxNew/lb.png">
					<img class="corner rb" src="${ctx}/static/image/cityDataCenter/jxNew/rb.png">
					<div class="tabCaptionBar">
						<span class="tabCaption">项目一览</span>
					</div>
					<div class="tabContBar">
			...
			<div class="blockStyle" style="height: calc(50% - 10px - 35px);">
				<div class="outsideBox">
					<img class="corner lt" src="${ctx}/static/image/cityDataCenter/jxNew/lt.png">
					<img class="corner rt" src="${ctx}/static/image/cityDataCenter/jxNew/rt.png">
					<img class="corner lb" src="${ctx}/static/image/cityDataCenter/jxNew/lb.png">
					<img class="corner rb" src="${ctx}/static/image/cityDataCenter/jxNew/rb.png">
					<div class="tabCaptionBar">
						<span class="tabCaption" style="width: 100%;">工程项目实时动态</span>
					</div>
					<div class="tabContBar">
						<div style="height:100%;overflow:hidden;">
...
function progressCount(){
	$.ajax({
		url : "${OfficeUrl}XOffice/interface/json/projectSchedule/progressCount",
		type : "post",
		data : {
			"orgId" : $('#orgId').val(),
			"orderType" : $('#kind').val()
...
function projectScheduleList(){
	$.ajax({
		url : "${OfficeUrl}XOffice/interface/json/projectSchedule/projectScheduleList",
		type : "post",
		data : {
			"orgId" : $('#orgId').val(),
			"orderType" : $('#kind').val()
			...
			if (result.success) {
				...
				if(result.data.projectList != "" && result.data.projectList != undefined){
					for(var j =0 ; j<4 ; j++){
						for(var i=0 ; i<result.data.projectList.length ; i++){
							var info = result.data.projectList[i];
							var url = "/login/page/projectSchedule/view?projectId="+info.id;
							var _div = "";
							_div = "<div class=\"projectRealData\" style=\"font-size: 16px;\">";
							_div+= "	<div class=\"projectRealDataTime\">"+nvlStr(info.parkName)+"</div>";
							_div+= "	<div class=\"projectRealDataName\" style=\"width: 100%;\" onclick='goDetail(\""+url+"\")'>"+nvlStr(info.projectName)+"</div>";
							_div+= "</div>";
							$("#projectBrowse").append(_div);
						}
					}
...
function industryCount(){
	$.ajax({
		url : "${OfficeUrl}XOffice/interface/json/projectSchedule/industryCount",
		type : "post",
		data : {
			"orgId" : $('#orgId').val(),
			"orderType" : $('#kind').val()
...
function goDetail(url){
	if(isLogin=='1'){ 
		...
		goPage("${ctx}/front/cityDataCenter/ycFourth/15?orgId=3609009")
	}
}
...
var rosePieOption = {
    ...
    color:['#B848EE','#FF4571','#FFCF53','#4EC971','#2199FF','#FF43CC'],
    series: [{
        type: 'pie',
        radius : ['55%', '70%'],
        center: ['40%', '50%'],
