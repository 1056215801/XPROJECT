				...
				<div class="row-box nobd main-box">
					...
					<div class="tab process-tab" style="overflow: hidden;">
						...
						<div class="process" id="processTab2" style="display:block;height: 100%;overflow: hidden;">
							...
							<div class="echart-map2" id="map2" onclick="toLink(4)"></div>
							<div class="companys">
...
var width = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
	$(function () {	
		...
		$("#selectMap p").on("click",function(){
			var ind = $(this).index();
			$(this).addClass("on").siblings().removeClass("on");
			if(ind == 0){ getCompany('上游')}
			if(ind == 1){ getCompany('中游')}
			if(ind == 2){ getCompany('下游')}
		});
	...
	function CreateGoodsDstribution() {
		$.get('${ctx}/static/map/360900.json', function(chinaJson) {
		    ...
		    var goodsMap = echarts.init(document.getElementById('map1'));
		  	var geoCoordMap = {
	  	    	'江西省': [116.054754,28.907192],
	  	    	'南昌市': [116.054754,28.907192],
	  	        '景德镇市': [117.17936,29.28948],
	  	        '萍乡市': [113.826452,27.696667],
	  	        '九江市': [116.068359,29.614163],
	  	        '新余市': [114.893496,27.693005],
	  	        '鹰潭市': [117.027582,28.317168],
	  	        '赣州市': [115.560027,27.452163],
	  	        '吉安市': [114.967085,27.154901],
	  	        '抚州市': [116.537102,27.634297],
	  	        '上饶市': [117.910652,28.528612],
	  	        '宜春市': [114.400262,27.911053],
	  	        '经开区': [114.072287,27.958528],
				'袁州区' : [114.25626,27.741851],
				'奉新县' : [ 115.38, 28.7 ],
				'万载县' : [ 114.43, 28.12 ],
				'上高县' : [ 114.92, 28.23 ],
				'宜丰县' : [ 114.56, 28.48 ],
			...
			var BJData = [
				[{name:'宜春市'},{name:'经开区',value:1}],
				[{name:'宜春市'},{name:'袁州区',value:44.78}],
				[{name:'宜春市'},{name:'宜丰县',value:144.06}],
				[{name:'宜春市'},{name:'奉新县',value:62.42}],
				[{name:'宜春市'},{name:'靖安县',value:1}],
				[{name:'宜春市'},{name:'高安市',value:6.64}]
			];
			var YCData = [
				[{name:'江西省'},{name:'宜春市',value:1}],
				[{name:'江西省'},{name:'新余市',value:1}],
				[{name:'江西省'},{name:'赣州市',value:1}]
			];
		    var yc = { name:'宜春市', coord: [114.400262,27.911053]}
		    var jx = { name:'江西省', coord: [116.054754,28.907192]}
			...
			var convertData = function (data,t) {
			    var res = [];
			    for (var i = 0; i < data.length; i++) {
			        var dataItem = data[i];
			        var fromCoord = t==1 ? yc.coord : jx.coord;
			        var toCoord = geoCoordMap[dataItem[1].name];
			        var cur = dataItem[1].name == '经开区' || dataItem[1].name == '宜春市' || dataItem[1].name == '高安市' || dataItem[1].name == '奉新县' ? 0.3 : -0.3
			        if (fromCoord && toCoord) {
			            res.push({
			                toName: t==1 ? yc.name : jx.name,
	                		fromName: dataItem[1].name,
			                coords: [toCoord, fromCoord],
			                value: dataItem[1].value,
			                lineStyle: {
			                	width: type==1 ? 6-i : i==0 ? 6 : 2,
	                			curveness: cur
			                },
			                effect: {
			                	symbolSize: type == 1 ? 25-(i > 3 ? 3 : i)*5 : i==0 ? 30 : 15
			                }
			            });
			        }
			    }
			...
			for(var type = 1;type < 3;type++){
				[['宜春市', type==1 ? BJData : YCData]].forEach(function (item, i) {
				    series.push(
				    {
				        type: 'lines',
				        zlevel: 2,
				        ...
				        effect: {
				            ...
				            color: {
				                ...
				                r: 0.5,
				                colorStops: [{
				                    offset: 0.4, color: '#fff' // 0% 处的颜色
				                }, {
				                    offset: 0.6, color: 'rgba(255,255,255,0.2)' // 100% 处的颜色
				                }, {
				        ...
				        lineStyle: {
			                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
			                        offset: 0.2, color: type==1?'#fff':'#FF3E3E'
			                    }, {
			                        offset: 0.6, color: type==1?'#00FFFC':'#FFCC00'
			                    }], false),
			                curveness: 0.3,
			                opacity : 1
				        },
				        data: convertData(item[1],type)
				    },
				    {
				        type: 'effectScatter',
				        ...
				        tooltip: {
				        	formatter: function(params){
				        		if(params.value[2] > 1){
					        		return '<p style="color: #1BE0E1">锂资源储量</p><b style="font-size:18px;">'+params.value[2]+'</b><b style="font-size:12px;">万吨</b>'
				        		}
				        	}
				        },
				        label: {
				            normal: {
				                ...
				                position: 'bottom',
				                fontWeight: 'bold',
				                fontSize: type == 1 ? fz4 : fz3,
				                formatter: '{b}',
				                color: type == 1 ? '#00FFF6' : '#FFCC00'
				            }
				        },
				        ...
				        itemStyle: {
			                color: type == 1 ? '#00FFF6' : '#FFD83E'
				        },
				        data: item[1].map(function (dataItem) {
				        	if(dataItem[1].name == "宜春市"){
				        		return {
					                name: dataItem[1].name,
					                value: geoCoordMap[dataItem[1].name].concat([dataItem[1].value]),
					                itemStyle: { color: '#00FFF6'},
					                rippleEffect: { scale: 6},
					                label: { fontSize: fz2, offset: [0,5]}
					            }
				        	} else if(dataItem[1].name == "经开区" || dataItem[1].name == "袁州区") {
				        		return {
					                name: dataItem[1].name,
					                value: geoCoordMap[dataItem[1].name].concat([dataItem[1].value]),
				    ...
				    },
				    {
				        ...
				        itemStyle: {
			                color: type == 1 ? {
				                ...
				                r: 0.5,
				                colorStops: [{
				                    offset: 0, color: 'rgba(255,216,62,.6)' // 0% 处的颜色
				                }, {
				                    offset: 1, color: 'rgba(255,216,62,0)' // 0% 处的颜色
				                }],
				                globalCoord: false // 缺省为 false
			                },
	                		opacity: .5
			...
			var y = {
				name:'江西省',
				value: [116.054754,28.907192,1],
                rippleEffect: { scale: 6},
                itemStyle: { color: '#FF3E3E'},
                label: { fontSize: fz2, position: 'top', offset: [0,-5]}
			}
			option.series[4].data.push(y);
			var y = {
				name:'江西省',
				value: [116.054754,28.907192,1],
                symbolSize: 120,
				itemStyle: { color: {
	                ...
	                r: 0.5,
	                colorStops: [{
	                    offset: 0, color: 'rgba(255,62,62,1)' // 0% 处的颜色
	                }, {
	                    offset: 1, color: 'rgba(255,62,62,0)' // 0% 处的颜色
	                }],
	                globalCoord: false // 缺省为 false
                }}
			}
			var point = {
		        ...
		        data: [
		        	...
		        	{
		        		name: '宜春市',
		        		value: [114.405262,27.905053,257.9],
		                itemStyle: { color: '#fff'}
		        	}
		        ]
			}
			option.series[5].data.push(y);
