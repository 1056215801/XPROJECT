...
<script type="text/javascript">
	...
	function table(){
		var year = $("#year").val();
		var month = $("#month").val();
		window.location.href="${ctx}/front/cityDataCenter/index/7?orgId=${orgId}&year="+year+"&month="+month;
	}
	...
	function loadBar1(data, sort){
		...
		if(data!='undefined'&&(!$.isEmptyObject(data.bar1Series1)
			...
			for(var i=0;i<3;i++){
				...
				selected.push(legendIndex);
			}
			if(year==2018){
				optionPart1.legend.selected= {'2018':true,'2017':false,'2016':false};
			}else if(year==2017){
				optionPart1.legend.selected= {'2017':true,'2016':false,'2015':false};
			}else if(year==2016){
				optionPart1.legend.selected= {'2016':true,'2015':false,'2014':false};
			}else if(year==2015){
				optionPart1.legend.selected= {'2015':true,'2014':false,'2013':false};
			}
	...
	function loadBar2(data, sort){
		...
		if(data!='undefined'&&!$.isEmptyObject(data.bar2Series1)
			...
			optionPart2.title.text=year+"年"+month+"月各产业"+valueName+"总量及同比增幅（单位：亿元）";
	...
	function loadBar3(compIndData) {
		...
		if(compIndData!='undefined'&&!$.isEmptyObject(compIndData)){
			//园区折线图
	...
	function loadPie(data, value){
		...
		if(data!='undefined'&&!$.isEmptyObject(data.pieSeries)){
			var pieOpion = {
					title: {
						show : true,
						text : '',
			...
			$("#pieSpan").show();
			$("#pieTitle").text(year+"年"+month+"月各产业"+valueName+"总量及占比（单位：亿元）");
			...
			pieOpion.title.text=total.toFixed(2);
			...
			$.ajax({
				...
				type:"post",
				data:{"orgId":orgId,"year":year,"month":month,"value":value+"_percent"},
				success:function(result){
					...
					if(result.success&&list!=null&&list!=""){
						...
						option.title.text=year+"年"+month+"月"+valueName+"增幅";
	...
	var optionPart1 = {
		    ...
		    legend: {
		    	top:'5%',
		    	selected:{'${info.year}':true,'${info.year-1}':false,'${info.year-2}':false},
		        data:[
